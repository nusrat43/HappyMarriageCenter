		...
		<div class="edit" style="align-items: center;">
			<div class="edit  login-form-w3-agile">
				...
				if (isset($_POST['submit'])) {
					...
					$email = $_POST['Email'];
					$sql = "UPDATE `register` SET `Profile_For`='$profile_for' ,`Name`='$name',`Gender`='$gender',`Email`='$email',`D_O_B`='$Date_of_birth',`Religion`='$religion' , `Country`='$country' where Email='$Email'";
					//echo  $sql;

					if ($conn->query($sql)) {
						$sql = "UPDATE `login` SET `Email`='$email' where Email='$Email'";
						$conn->query($sql);
						echo "<script>
